//
//  main.cpp
//  String_Anagram_in_other_string
//
//  Created by MLS Mac 1 on 28/05/20.
//  Copyright © 2020 MLS Mac 1. All rights reserved.
//


/*Logic::In this question we need to get if the permuation/anagram of s1 is present in s2.. s1== ecd and s2 ... abcde..so yes
 ...
 If the length of window is equal to size of s1..then check last charcter count is same or not..if same then yes this is valid anagram..else remove element from left
...
StringVec_t checkInclusionPerm(string s1, string s2)
{
    ...
    while(r<len2)
    {
        ...
        if(total_elem==len1 )
        {
            ...
            if(count1==count2)
            {
                string anagram = s2.substr(l,r-l+1);
                //std::cout <<"\n anagram string=" <<anagram ;
                res_vec.push_back(anagram);
...
class Solution {
public:
    StringVec_t checkInclusion(string s1, string s2) {
        
      StringVec_t result =   checkInclusionPerm(s1,s2);
